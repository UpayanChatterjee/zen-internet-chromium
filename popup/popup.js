...
new (class ExtensionPopup {
  ...
  async loadSettings() {
    ...
    const globalData = await chrome.storage.local.get(this.BROWSER_STORAGE_KEY);
    ...
    if (this.currentSiteHostname) {
      ...
      const normalizedData = await chrome.storage.local.get(normalizedSiteKey);
      const originalData = await chrome.storage.local.get(originalSiteKey);
  ...
  async loadSkipForceThemingList() {
    const data = await chrome.storage.local.get(SKIP_FORCE_THEMING_KEY);
  ...
  async loadSkipThemingList() {
    const data = await chrome.storage.local.get(SKIP_THEMING_KEY);
  ...
  async loadFallbackBackgroundList() {
    const data = await chrome.storage.local.get(FALLBACK_BACKGROUND_KEY);
  ...
  async loadCurrentSiteFeatures() {
    ...
    try {
      const stylesData = await chrome.storage.local.get("styles");
      ...
      if (!currentSiteKey) {
        ...
        const userMappingData = await chrome.storage.local.get(
        ...
        for (const [sourceStyle, targetSites] of Object.entries(
          ...
          if (targetSites.includes(this.normalizedCurrentSiteHostname)) {
            ...
            console.log(
              `Found mapped style: ${sourceStyle} for ${this.normalizedCurrentSiteHostname}`
            );
            break;
          }
        }
      ...
      for (const [feature, css] of Object.entries(features)) {
        ...
        const featureId = `feature-desc-${currentSiteKey.replace(
          /[^a-zA-Z0-9]/g,
          "_"
        )}-${displayFeatureName.replace(/[^a-zA-Z0-9]/g, "_")}`;
  ...
  async refetchCSS() {
    ...
    try {
      ...
      if (hasNewMappings) {
        ...
        console.log(
          "Popup: Using new mappings from repository:",
          styles.mapping
        );
      } else {
  ...
  async applyCSSToTab(tab) {
    ...
    try {
      ...
      try {
        await chrome.tabs.removeCSS(tab.id, {
      ...
      if (bestMatch) {
        ...
        if (combinedCSS) {
          await chrome.tabs.insertCSS(tab.id, { code: combinedCSS });
        ...
        if (this.globalSettings.forceStyling) {
          ...
          if (shouldApplyForcedStyling && styles["example.com.css"]) {
            ...
            const siteData = await chrome.storage.local.get(siteStorageKey);
  ...
  setupAutoUpdate() {
    ...
    if (this.autoUpdateSwitch.checked) {
      chrome.runtime.sendMessage({ action: "enableAutoUpdate" });
    } else {
      chrome.runtime.sendMessage({ action: "disableAutoUpdate" });
    }
  }

  displayLastFetchedTime() {
    if (logging) console.log("displayLastFetchedTime called");
    chrome.storage.local.get(this.BROWSER_STORAGE_KEY).then((result) => {
