document.addEventListener("DOMContentLoaded", function () {
  ...
  async function loadRepositoryUrl() {
    try {
      const data = await chrome.storage.local.get(REPOSITORY_URL_KEY);
  ...
  async function setRepositoryUrl() {
    try {
      ...
      await chrome.storage.local.set({ [REPOSITORY_URL_KEY]: newUrl });
  ...
  async function saveFeatureToggle(websiteDomain, feature, isEnabled) {
    try {
      ...
      await chrome.storage.local.set({ [siteKey]: siteSettings });
        ...
        try {
          ...
          if (
            ...
            if (importData.userMappings) {
              importOperations[USER_STYLES_MAPPING_KEY] =
                importData.userMappings;
            }

            // Apply all settings at once
            await chrome.storage.local.set(importOperations);
  ...
  async function loadUserMappingsUI() {
    ...
    const data = await chrome.storage.local.get(USER_STYLES_MAPPING_KEY);
        ...
        .forEach((btn) => {
          btn.addEventListener("click", async (e) => {
            ...
            if (userMapping.mapping[source]) {
              userMapping.mapping[source] = userMapping.mapping[source].filter(
                (s) => s !== site
              );
